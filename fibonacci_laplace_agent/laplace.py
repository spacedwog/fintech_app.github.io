...
def ml_recognition(regression_table, limiar_anomalia=0.2):
    ...
    for cat, serie in regression_table.items():
        ...
        anomalias = [
            ...
            and abs(p["realizado"] - p["previsto"]) / p["previsto"] > limiar_anomalia
        ]
...
def ml_testing(regression_table, holdout=1):
    ...
    for cat, serie in regression_table.items():
        ...
        if n <= holdout:
            ...
            continue
        treino, teste = serie[:-holdout], serie[-holdout:]
        a, b = _ols(list(range(len(treino))), [p["realizado"] for p in treino])
...
def ml_learning(regression_table, novo_ponto_por_categoria=None):
    ...
    return atualizada, ml_training(atualizada)
...
class LaplacePipeline:
    def __init__(self, rows_passado):
        self.passado = build_regression_table(rows_passado)
        self.presente = {}
        self.futuro = {}

    def rodar_presente(self, holdout=1, novo_ponto_por_categoria=None):
        atualizada, learning = ml_learning(self.passado, novo_ponto_por_categoria)
        self.presente = {
            "recognition": ml_recognition(self.passado),
            "training": ml_training(self.passado),
            "testing": ml_testing(self.passado, holdout=holdout),
            "learning": learning,
        }
    ...
    def prever_proximo(self):
        ...
        modelos = self.presente.get("training") or ml_training(self.passado)
        return {
            cat: round(prever(modelos[cat], len(serie)), 2)
