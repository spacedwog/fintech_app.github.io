...
(function (global) {
  ...
  function loadTesseract() {
    ...
    if (loadTesseract._pending) return loadTesseract._pending;

    loadTesseract._pending = new Promise(function (resolve, reject) {
    ...
    return loadTesseract._pending;
  }

  function classify(text, amounts, merchantMatches, ctx) {
    ...
    if (amountMatches && expectedType) {
      ...
      for (var j = 0; j < REFERENCE_AMOUNTS.length; j++) {
        if (closeEnough(detectedAmount, REFERENCE_AMOUNTS[j].amount)) {
          classification = REFERENCE_AMOUNTS[j].type;
          break;
        }
      }
    }
  ...
  function analyze(file, ctx) {
    ...
    return loadTesseract()
      ...
      .then(function (result) {
        ...
        var merchantMatches =
          digits.indexOf(MERCHANT_CNPJ_DIGITS) !== -1 || letters.indexOf(MERCHANT_NAME) !== -1;

        var amounts = parseAmounts(text);
        var c = classify(text, amounts, merchantMatches, ctx);
